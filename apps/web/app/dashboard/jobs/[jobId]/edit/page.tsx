...
import { Loader2, ArrowLeft, Save, Plus, Trash2, DollarSign, Award, Settings } from "lucide-react";
...
export default function EditJobPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  useEffect(() => {
    getJob(jobId)
      .then((data) => {
        ...
        setSalaryMin(job.salaryRangeMin ? String(job.salaryRangeMin) : "");
        setSalaryMax(job.salaryRangeMax ? String(job.salaryRangeMax) : "");
        setLoading(false);
      })
      .catch((err) => {
  ...
  const removeSkill = (idx: number) => {
    if (skills.length === 1) {
      toast.error("You must specify at least one skill parameter.");
      return;
    }
  ...
  const handleNext = () => {
    ...
    if (step === 2) {
      ...
      if (hasEmptySkill) {
        toast.error("Please ensure all skill parameters are completed.");
        return;
      }
    }
    setStep(step + 1);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Failed to update job requisition:", err);
      toast.error(err.message || "Failed to save requisition changes.");
    } finally {
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground animate-pulse">Syncing job requisition dossier...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div>
        <h2 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">Edit Requisition</h2>
        <p className="text-sm text-muted-foreground mt-1">Modify structural match parameters and compensation criteria for requisition designation: {title}</p>
      </div>

      {/* Steps tracking progress bar */}
      ...
      <Card className="border-border/40 shadow-sm relative overflow-hidden">
        <CardHeader className="border-b border-border/20 bg-muted/10">
          <CardTitle className="text-base font-bold flex items-center gap-2">
            ...
            {step === 1 ? "Edit Basic Information" : step === 2 ? "Edit Role Specifications" : "Edit Compensation Details"}
          </CardTitle>
